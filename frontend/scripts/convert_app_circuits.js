...
// Filter files for PNG, JPG, or JPEG
...
async function convertAll() {
  ...
  for (const file of pngFiles) {
    // Skip certificate/non-product files
    if (file.includes('Google Digital Marketing')) {
      console.log(`ℹ️ Skipping non-product file: "${file}"`);
      continue;
    }
    ...
    if (matchedProduct) {
      ...
      // The app circuit filename is defined in the database
      const appCircuitName = matchedProduct['App Circuit Image Filename'] || (matchedProduct['Part#'].toLowerCase().replace(/[^a-z0-9]/g, '') + '-application-circuit.webp');
      const destPath = path.join(publicDir, appCircuitName);

      try {
        console.log(`🔄 Converting: "${file}" -> "${appCircuitName}"...`);
